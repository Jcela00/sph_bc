...
for k = 1:length(steps)
    ...
    E_10 = ComputeErrorAndPlotTC(set_10, 0);
    E_15 = ComputeErrorAndPlotTC(set_15, 0);
    E_20 = ComputeErrorAndPlotTC(set_20, 0);
    E_30 = ComputeErrorAndPlotTC(set_30, 0);
    % E_40 = ComputeErrorAndPlotTC(set_40, 0);

    % Accumulate Error of the different x1 x2 x3 refinements (and old) for the different resolutions
    E_1(k, :) = [E_10(1), E_15(1), E_20(1), E_30(1)];
    E_2(k, :) = [E_10(2), E_15(2), E_20(2), E_30(2)];
    E_3(k, :) = [E_10(3), E_15(3), E_20(3), E_30(3)];
...
semilogy(steps, E_2(:, 1), 'o-');
semilogy(steps, E_3(:, 1), 'o-');
title('10'); legend('x1', 'x2', 'x3');
figure; hold on;
semilogy(steps, E_1(:, 2), 'o-');
semilogy(steps, E_2(:, 2), 'o-');
semilogy(steps, E_3(:, 2), 'o-');
title('20'); legend('x1', 'x2', 'x3');
figure; hold on;
semilogy(steps, E_1(:, 3), 'o-');
semilogy(steps, E_2(:, 3), 'o-');
semilogy(steps, E_3(:, 3), 'o-');
title('30'); legend('x1', 'x2', 'x3');

averaging_window = [1 21];
...
p2 = polyfit(log(res(1:3)), log(E_old_avg(1:3)), 1);
...
plot(log(res(1:3)), (polyval(p2, log(res(1:3)))), 'r--');
text(log(res(2)) * 0.9, log(E_1_avg(2)), ['Slope: ' num2str(p1(1)) '         '], 'HorizontalAlignment', 'right');
text(log(res(2)) * 1.1, log(E_old_avg(2)), ['         Slope: ' num2str(p2(1))], 'HorizontalAlignment', 'left');
ylabel('$L_2$ error');
xlabel('$N_{particles}$');
