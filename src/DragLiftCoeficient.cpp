                  ...
                  size_t write)
{
    ...
    obstacle_force_x = obstacle_force_x / (params.eta * vx);
    ...
    if (v_cl.getProcessUnitID() == 0)
    {
        if (write == 0)
        {
            avgvelstream << "t, vx, vy, v, vtx, vty, vt, drag, lift" << std::endl;
        }
        // avgvelstream << "t = " << t << ", vx = " << vx << ", vy = " << vy << ", v = " << v << ", vtx = " << vtx << ", vty = " << vty << ", vt = " << vt << ", drag = " << obstacle_force_x << ", lift = " << obstacle_force_y << std::endl;
        avgvelstream << t << "," << vx << "," << vy << "," << v << "," << vtx << "," << vty << "," << vt << "," << obstacle_force_x << "," << obstacle_force_y << std::endl;
    }
}
