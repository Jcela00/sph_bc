            ...
            if (r2 < _params_gpu_.r_cut2 && r2 > 1e-16)
            {
                ...
                if (typeb == BOUNDARY || typeb == OBSTACLE)
                {
                    ...
                    for (int i = 0; i < 3; i++) // for the 3 boundary particles
                    {
                        ...
                        const Point<DIM, real_number> v_bound_aux = ((vwn - vn) * (lwall[i] / lf) + vwn) * normal + vt * tangential;
                        ...
                        const Point<DIM, real_number> v_rel_aux = va - v_bound_aux;
                        ...
                        {
                            vd.template getProp<vd3_drho>(a) += rhoa * massb * dotProduct(v_rel_aux, DW) / rho_boundary;
                        }
                        // }
                    }
                }
                        ...
                        const Parameters &params)
{
    ...
    {
        vd.template updateCellListGPU<vd0_type, vd1_rho, vd2_pressure, vd3_drho, vd4_velocity, vd5_velocity_t, vd6_force, vd7_force_t>(NN);
