...
{
    const real_number timetosteady = 1.0;
    if (t <= timetosteady)
...
__global__ void kdi_1_gpu(vd_type vd, const real_number dt, const real_number t)
{
    ...
    else if (vd.template getProp<vd0_type>(a) == FLUID) // Otherwise it is a fluid particle
    {
        if (_params_gpu_.SCENARIO == CUSTOM)
        {
            ...
            {
                const real_number fac(_params_gpu_.SCENARIO == CUSTOM ? VelocityRamp(t) : 1.0);
                for (int xyz = 0; xyz < DIM; xyz++)
                {
        ...
        if constexpr (FREEZE_DAM == 1)
        {
            if (pos.get(0) < _params_gpu_.length[0])
            {
                if (t > 0.9)
                {
                    for (int xyz = 0; xyz < DIM; xyz++)
